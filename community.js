...
async function openProfile(profileId) {
  ...
  const { data: redirectProfile } = await db
    .from("profiles")
    .select("username")
    .eq("id", profileId)
    .maybeSingle();
  window.location.href = redirectProfile?.username
    ? `/${encodeURIComponent(redirectProfile.username)}`
  ...
  const [
    ...
    db
      .from("profiles")
      .select(
        "id, username, display_name, bio, avatar_url, is_vip, blue_tick, gold_tick, community_role, created_at",
      )
      .eq("id", profileId)
      .single(),
    db
  ...
  if (error || postError || followerError || followingError)
    throw error || postError || followerError || followingError;
  if (person.privacy === "private" && profileId !== user.sub) {
    ...
    if (!access) {
      $("#profileDetails").innerHTML =
        `<section class="instagram-profile"><img class="profile-hero-avatar" src="${avatar(person)}" alt="" /><div><p class="eyebrow">Private account</p><h2>${escapeHtml(person.display_name)} ${badge(person)}</h2><p class="profile-handle">@${escapeHtml(person.username)}</p><p>This profile is visible to approved followers only.</p></div></section>`;
...
async function loadPosts() {
  const { data, error } = await db
    .from("posts")
    .select(
      "id, author_id, body, image_url, created_at, profiles!posts_author_id_fkey(id, username, display_name, avatar_url, blue_tick, gold_tick, community_role)",
    )
    .order("created_at", { ascending: false })
    .limit(30);
  if (error) throw error;
...
async function searchPeople(query = "") {
  ...
  const [{ data: people, error }, { data: following, error: followError }] =
    await Promise.all([
      request,
...
$("#postForm").addEventListener("submit", async (event) => {
  ...
  try {
    ...
    const { error } = await db
      .from("posts")
      .insert({ author_id: user.sub, body, image_url: imageUrl });
    if (error) throw error;
    form.reset();
    await loadPosts();
