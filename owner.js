...
const loadSpecialDayControls = async () => { const settings = await ownerRequest("get-site-settings"); for (const [id, key] of [["specialDayStart", "special_day_start"], ["specialDayEnd", "special_day_end"], ["specialDayName", "special_day_name"], ["specialDayTitle", "special_day_title"], ["specialDayMessage", "special_day_message"]]) document.querySelector(`#${id}`).value = settings[key] || ""; document.querySelector("#specialDayEnabled").checked = settings.special_day_enabled === true; document.querySelector("#specialDayTheme").value = settings.special_day_theme || "sakura"; };
specialDayForm?.addEventListener("submit", async (event) => { event.preventDefault(); try { await ownerRequest("update-site-settings", { special_day_enabled: document.querySelector("#specialDayEnabled").checked, special_day_start: document.querySelector("#specialDayStart").value, special_day_end: document.querySelector("#specialDayEnd").value, special_day_name: document.querySelector("#specialDayName").value.trim(), special_day_title: document.querySelector("#specialDayTitle").value.trim(), special_day_message: document.querySelector("#specialDayMessage").value.trim(), special_day_theme: document.querySelector("#specialDayTheme").value }); ownerStatus.textContent = "Special day mood saved. ✦"; } catch (error) { ownerStatus.textContent = error.message; } });
