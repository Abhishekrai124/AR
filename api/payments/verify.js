...
export default async function handler(request, response) {
  ...
  if (!secret || !orderId || !paymentId || !signature)
    return response.status(400).json({ error: "Invalid payment response." });
  ...
  if (keyId) {
    ...
    const order = await orderCheck.json();
    if (
      !orderCheck.ok ||
      Number(order.amount) < 1000 ||
      Number(order.amount) > 1000000 ||
      order.status !== "paid"
    )
      return response
        .status(400)
        .json({ error: "Order could not be confirmed." });
  }
  const bearer = request.headers.authorization || "";
  const supabaseUrl =
    process.env.SUPABASE_URL || "https://atphyjukjgnnbfbnizyx.supabase.co";
  const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!bearer.startsWith("Bearer ") || !serviceKey)
    return response.status(200).json({ verified: true, activation: "pending" });
  ...
  if (product === "vip" && auth.ok && user.id)
    await fetch(
