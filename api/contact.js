...
export default async function handler(request, response) {
  ...
  if (!bearer.startsWith("Bearer ")) return response.status(401).json({ error: "Sign in to view personal contact details." });
  try {
    const userResponse = await fetch(`${supabaseUrl}/auth/v1/user`, { headers: { apikey: anonKey, Authorization: bearer } });
    const user = await userResponse.json();
    if (!userResponse.ok || !user.email) return response.status(401).json({ error: "Your session could not be verified." });
    ...
    return response.status(502).json({ error: "Personal contact details are temporarily unavailable." });
  }
}
