...
export default async function handler(request, response) {
  ...
  try {
    ...
    if (!session.ok || !user?.id)
      throw new Error("Your session could not be verified.");
    const profileDeleted = await fetch(
    ...
    if (!profileDeleted.ok)
      throw new Error("Your community data could not be deleted.");
    ...
    if (!deleted.ok)
      throw new Error(
        "Your profile was removed, but your sign-in could not be deleted. Please contact support.",
      );
    return response.status(200).json({ ok: true });
  } catch (error) {
    return response
      .status(
        error.message.includes("sign in") || error.message.includes("verified")
          ? 403
          : 500,
      )
      .json({ error: error.message || "Account request failed." });
  }
}
