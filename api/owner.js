...
export default async function handler(request, response) {
  ...
  try {
    ...
    if (request.body.action === "update-site-settings") {
      ...
      if (request.body.special_day_image_url !== undefined) changes.special_day_image_url = String(request.body.special_day_image_url).slice(0, 2000);
      for (const key of ["special_day_name", "special_day_title", "special_day_message", "special_day_role", "special_day_tags", "special_day_links", "special_day_intro_title", "special_day_intro_text", "special_day_cta_title"]) if (request.body[key] !== undefined) changes[key] = String(request.body[key]).slice(0, 2000);
      ...
      changes.id = "global";
      const upstream = await fetch(`${supabaseUrl}/rest/v1/site_settings?on_conflict=id`, { method: "POST", headers: { ...adminHeaders(), Prefer: "resolution=merge-duplicates,return=minimal" }, body: JSON.stringify(changes) });
      if (!upstream.ok) { const detail = await upstream.text(); throw new Error(`Could not update site settings (${upstream.status}). ${detail.slice(0, 180)}`); }
      return response.status(200).json({ ok: true });
    }
